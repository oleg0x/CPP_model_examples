...
int func(int i, const char* str) { printf("%d) %s\n", i, str); return 0; }

int add(int a, int b) { printf("%d + %d = %d\n", a, b, a + b); return a + b; }
int subtract(int a, int b) { printf("%d - %d = %d\n", a, b, a - b); return a - b; }
int multiply(int a, int b) { printf("%d * %d = %d\n", a, b, a * b); return a * b; }
...
int main()
{
	...
	
	{
		...
		int* p1 = &n;
		int* p2 = p1;
		if ( n == *p1 && p1 == p2 )  puts("Equal");
	}
	
	{
		int arr[10];
		int* p1 = arr;
		int* p2 = &arr[0];  // This is the same as the previous line
